  ...
  while (1) {
    ...
    if (L_CANON(tty)) {
      if ((KILL_CHAR(tty) != _POSIX_VDISABLE) && (c == KILL_CHAR(tty))) {
        ...
        while (
            !(EMPTY(tty->secondary) || (c = LAST(tty->secondary)) == 10 ||
...
static struct tty_queue init_tty_queue = {0, 0, 0, 0, ""};

void tty_init(void) {
  int i;

  for (i = 0; i < QUEUES; i++)
    memcpy(&tty_queues[i], &init_tty_queue, sizeof(struct tty_queue));

  init_tty_queue.data = 0x3f8;
  memcpy(&rs_queues[0], &init_tty_queue, sizeof(struct tty_queue));
  memcpy(&rs_queues[1], &init_tty_queue, sizeof(struct tty_queue));

  init_tty_queue.data = 0x2f8;
  memcpy(&rs_queues[3], &init_tty_queue, sizeof(struct tty_queue));
  memcpy(&rs_queues[4], &init_tty_queue, sizeof(struct tty_queue));
  ...
  for (i = 0; i < NR_CONSOLES; i++) {
    con_table[i] =
        (struct tty_struct){{ICRNL,         /* change incoming CR to NL */
  ...
  for (i = 0; i < NR_PTYS; i++) {
    mpty_table[i] = (struct tty_struct){{0, /* no translation */
                                        ...
                                        mpty_queues + 2 + i * 3};
    spty_table[i] =
        (struct tty_struct){{0, /* no translation */
                             0, /* no translation */
                             B9600 | CS8, IXON | ISIG | ICANON, 0, INIT_C_CC},
                            0,
                            0,
                            0,
                            spty_write,
