...
int do_execve(unsigned long *eip, long tmp, char *filename, char **argv,
  ...
  task_ctl_current->bits &= ~(TASK_CTL_INIT_FORK);
...
restart_interp:
  ...
  if (!sh_bang) {
    task_ctl_current->bits &= ~TASK_CTL_SH_BANG;
    p = copy_strings(envc, envp, page, p, 0);
  ...
  memset(task_ctl_current->filename, 0, sizeof(task_ctl_current->filename));
  memset(task_ctl_current->argv, 0, sizeof(task_ctl_current->argv));
  memset(task_ctl_current->envp, 0, sizeof(task_ctl_current->envp));

  // 保存 execve 结果信息便于调试
  unsigned long oldp = p;
  unsigned long oldargc = argc;
  unsigned long oldenvc = envc;
  char *src = filename;
  char *dst = task_ctl_current->filename;
  i = 31;
  int j;

  while (((*dst++) = get_fs_byte(src++)) && i--)
    ;
  *dst = '\0';

  src = ((char *)page[p / PAGE_SIZE]) + (p % PAGE_SIZE);

  j = 0;
  while (argc--) {
    if (j < 32) {
      dst = task_ctl_current->argv[j];
    }
    i = 31;
    while (*src) {
      if (j < 32 && i--) {
