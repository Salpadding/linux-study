...
#define copy_page(from, to)                                                    \
  __asm__("movl %0, %%esi\n\t"                                                 \
          "movl %1, %%edi\n\t"                                                 \
          "movl %2, %%ecx\n\t"                                                 \
          "cld ; rep ; movsl" ::"rm"(from),                                    \
          "rm"(to), "rm"(1024)                                                 \
          : "edi", "esi", "ecx")

#define CHECK_LAST_NR 16
static unsigned long last_pages[CHECK_LAST_NR];
    ...
    for (nr = 0; nr < 1024; nr++, pg_table++) {
      ...
      *pg_table = 0;
      if (1 & page)
        ...
        swap_free(page >> 1); // free page which had been swapped out
    }
  ...
  for (; size-- > 0; from_dir++, to_dir++) {
    ...
    *to_dir = ((unsigned long)to_page_table) | 7;
    ...
    for (; nr-- > 0; from_page_table++, to_page_table++) {
      ...
      if (!this_page)
        continue;
      if (!(1 & this_page)) { // 需要浅拷贝的页面换出了 那就换入
                              // 然后被新的进程地址空间指向换出的位置
...
unsigned long put_dirty_page(unsigned long page, unsigned long address) {
  ...
  if (mem_map[(page - LOW_MEM) >> 12] != 1)
    printk("mem_map disagrees with %p at %p\n", page, address);
#ifdef CONFIG_PATCH
  page_table = PG_DIR_ENTRY(address);
  if ((*page_table) & 1) {
    *(PG_TBL_ENTRY(address)) = page | (PAGE_DIRTY | 7);
    return page;
  }
  if (!(tmp = get_free_page()))
    return 0;
...
void do_wp_page(unsigned long error_code, unsigned long address,
...
#ifdef CONFIG_PATCH
    panic("Bad things happen: page error in do_wp_page\n\r");
#else
  ...
  if (task_ctl_current->bits & TASK_CTL_INIT_FORK) {
    panic("unexpected wp page under init fork\n");
  }
  un_wp_page(PG_TBL_ENTRY(address));
}

void write_verify(unsigned long address) {
  unsigned long page;

  if (!((page = *(PG_DIR_ENTRY(address))) & 1))
    return;
  page = (unsigned long)PG_TBL_ENTRY(address);
...
static int try_to_share(unsigned long address, struct task_struct *p) {
  ...
  from = *(unsigned long *)(from_page + UL(pg_dir));
  if (!(from & 1))
    return 0;
  from &= 0xfffff000;
  ...
  if (phys_addr >= HIGH_MEMORY || phys_addr < LOW_MEM)
    return 0;
  to = *(unsigned long *)(to_page + UL(pg_dir));
  if (!(to & 1))
    if (to = get_free_page())
...
void mem_init() {
  ...
  // 页表自映射
  pg_dir[1023] = (unsigned long)pg_dir | 7;

  // 加载页表
  __asm__ __volatile__("movl %0, %%cr3\n" : : "r"((unsigned long)pg_dir));
  ...
  // 把代码段设置为写保护 防止被栈溢出写坏
  unsigned long text_size = ((unsigned long)&_data) - ((unsigned long)&_text);

  text_size += PAGE_SIZE - 1;
  text_size /= PAGE_SIZE;

  unsigned long *pt = PG_TBL_ENTRY(&_text);
...
int mmap(unsigned long vstart, unsigned long pstart, unsigned long page_nr,
         unsigned long bits) {
  unsigned long *dir = PG_DIR_ENTRY(vstart);

  if (!*dir)
    *dir = get_free_page() | 7;
  unsigned long tmp;
  unsigned long *pt = PG_TBL_ENTRY(vstart);
...
/* This routine handles page faults.  It determines the address,
   ...
   routines. */
void do_page_fault(trap_stack_t stack) {
  unsigned long address;
  /* get the address */

  __asm__("movl %%cr2,%0" : "=r"(address));
  if (!(stack.error_code & 1)) {
    do_no_page(stack.error_code, address, current);
    return;
  } else {
    do_wp_page(stack.error_code, address, &stack);
    return;
  }
}
