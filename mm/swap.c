...
#define bitop(name, op)                                                        \
  static int name(char *addr, unsigned int nr) {                               \
    int __res;                                                                 \
 ...
 */
int swap_out(void) {
  static long dir_entry = FIRST_VM_PAGE >> 10;
  static long page_entry = -1; // index of current page entry
...
repeat:
  ...
  if (!(pg_table & 1)) { // impossible to be swapped out
    printk("bad page-table at pg_dir[%d]: %08x\n\r", dir_entry, pg_table);
    return 0;
  }
  ...
  while (counter-- > 0) {
    ...
    if (page_entry >= 1024) {
      page_entry = -1;
      goto repeat; // goto next directory
    }
...
repeat:
  __asm__("movl %2, %%ecx\n\t"
          ...
          "cld\n\t"
          : "+a"(result)
          : "i"(LOW_MEM), "r"(PAGING_PAGES), "r"(mem_map + PAGING_PAGES - 1)
          : "ecx", "edi", "edx");
...
int sys_swapon(const char *specialfile) {
  ...
  if (!(swap_inode = namei(specialfile)))
    return -ENOENT;
  if (swap_file || swap_device || swap_bitmap) { // already enabled
    iput(swap_inode);
    return -EBUSY;
  }
  ...
  read_swap_page(0, swap_bitmap); // read first page from file/partition
